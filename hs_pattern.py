...
def check_hs_pattern(extrema_indices: List[int], data: np.array, i: int, order: int, early_find: bool = False) -> HSPattern:
    ...
    # Find right shoulder as the highest confirmed local top since r_armpit (fix: was global argmax)
    candidates = [
        j - order
        for j in range(r_armpit + order + 1, i + 1)
        if rw_top(data, j, order)
    ]
    if not candidates:
        return None
    r_shoulder = max(candidates, key=lambda j: data[j])
    ...
    # Find beginning of pattern. Neck to left shoulder (fix: was head_width, now 2x for longer lead-ins)
    head_width = r_armpit - l_armpit
    pat_start = -1
    neck_start = -1
    for j in range(1, head_width * 2):
...
def check_ihs_pattern(extrema_indices: List[int], data: np.array, i: int, order: int, early_find: bool = False) -> HSPattern:
    ...
    # Find right shoulder as the lowest confirmed local bottom since r_armpit (fix: was global argmin)
    candidates = [
        j - order
        for j in range(r_armpit + order + 1, i + 1)
        if rw_bottom(data, j, order)
    ]
    if not candidates:
        return None
    r_shoulder = min(candidates, key=lambda j: data[j])
    ...
    pat.pattern_r2 = compute_pattern_r2(data, pat)  # fix: was assigned twice
...
def find_hs_patterns(data: np.array, order: int, early_find: bool = False):
    ...
    for i in range(len(data)):
        ...
        if ihs_lock or not ihs_alternating:
            ...
            ihs_pat = check_ihs_pattern(ihs_extrema, data, i, order, early_find)

        if hs_lock or not hs_alternating:
            hs_pat = None
        else:
            hs_pat = check_hs_pattern(hs_extrema, data, i, order, early_find)
...
def plot_hs(candle_data: pd.DataFrame, pat: HSPattern, pad: int = 2):
    ...
    # fix: use actual index midpoint for label x-position
    mid_i = pat.start_i + (pat.break_i - pat.start_i) // 2
    x = idx[mid_i]
    if pat.inverted:
...
if __name__ == '__main__':
    data = pd.read_csv('/Users/justindehn/Downloads/BTCUSDT3600.csv')
