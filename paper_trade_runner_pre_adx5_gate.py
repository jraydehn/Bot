    ...
    if auth is not None:
        ...
        for c in ladder:
            ...
            if args.asset == "BTC" and _composite_computed and sigma_tau_c > 0:
                ...
                # [ETH OTM YES gate] Block YES when strike > spot (OTM), pm < 0.45, AND p_up <= 0.50.
                # p_up > 0.50 (bullish model signal) overrides the gate — market underpricing upside
                # is exactly where edge exists. Gate only fires in bearish/neutral regimes.
                ...
                _eth_otm_yes_blocked = (
                    ...
                    and (_comp_p_up is None or _comp_p_up <= 0.50)
                )
                # When bullish (p_up > 0.50) and strike is OTM, force YES — don't let NO win
                # on edge comparison against a rising market.
                _eth_otm_no_blocked = (
                    args.asset == "ETH" and offset_c > 0
                    and _comp_p_up is not None and _comp_p_up > 0.50
                )
                if _eth_otm_yes_blocked:
                    print(f"  [eth_otm_yes_gate] BLOCK YES {c['ticker']} — pm={pm:.3f}<0.45, OTM offset={offset_c*100:+.2f}%, p_up={_comp_p_up:.3f}")
                if _eth_otm_no_blocked:
                    print(f"  [eth_otm_no_gate] FORCE YES {c['ticker']} — p_up={_comp_p_up:.3f}>0.50, OTM offset={offset_c*100:+.2f}%")
                dec_c = evaluate_trade(
                    ...
                    force_side="yes" if _eth_otm_no_blocked else ("no" if _eth_otm_yes_blocked else None))
            ...
            # BearDrift gate (BTC YES only): block when ema_stack=-1 + composite_rev<=3 + stoch_k>=35
            # Rescues: stoch_k<35 (oversold), composite_rev>=4 (strong reversal),
            #          vpin_score=1 (positive smart money flow), ema_stretch_score=1 (bullish stretch)
            if dec_c.decision == "trade" and args.asset == "BTC" and dec_c.side == "yes":
                ...
                _bd_not_os = (_bd_sk >= 35)
                if _bd_ema and _bd_rev and _bd_not_os:
                    _bd_rescued = confirm.vpin_score == 1 or confirm.ema_stretch_score == 1
                    if _bd_rescued:
                        _bd_why = "vpin=1" if confirm.vpin_score == 1 else "ema_stretch=1"
            ...
            # No rescue identified — no sub-slice above breakeven in exhaustive search.
            ...
            # composite_rev<=-4 (max reversal score), stoch>=75 (overbought), vwap_stretch=-1 (1-2σ above VWAP).
            # Rescue search exhaustive — no sub-condition with sound causal logic found. Flat block.
            # Backtest: n=19, WR=31.6%, BE=53.4%, Δ=-21.8pp, -$285 saved.
            if (dec_c.decision == "trade" and args.asset == "BTC"
                    and dec_c.side == "yes"
