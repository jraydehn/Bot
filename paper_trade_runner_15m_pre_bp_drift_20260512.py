...
def compute_p_model_15m(spot: float, floor_strike: float,
    ...
    Base: log-normal with drift from composite_p_up (hourly directional signal
    scaled to the 15m window via sqrt(tau/60)).
    Microstructure: bp_5m and body_15m × dir_15m only.
    """
    ...
    # Base z-score: current position relative to strike
    z_base = math.log(spot / floor_strike) / sigma_tau

    # Drift from composite_p_up: P(price up over ~1h) scaled to tau_min window.
    # norm.ppf(p_up) is the implied z-score at the 1h horizon; sqrt(tau/60)
    # scales it to the current contract's remaining time.
    ...
    # Microstructure delta: bp_5m and body_15m × dir_15m only
    delta = 0.0
