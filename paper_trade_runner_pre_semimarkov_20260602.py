...
def _vol1h_hmm_probs(live_1m: "pd.DataFrame") -> "tuple[int,float,float] | None":
    """Return (hard_rank, r1_prob, r1_prob_k10) from live 1m data (last 20 15m bars).
    ...
    try:
        ...
        obs      = lr.reshape(-1, 1)
        # Hard Viterbi rank
        raw_hard = int(_vol1h_hmm_model.predict(obs)[-1])
        hard_rank = _vol1h_hmm_rankof[raw_hard]
        # Soft posterior (forward-backward): shape (n_bars, n_states) in raw state order
        post_raw  = _vol1h_hmm_model.predict_proba(obs)[-1]   # last bar posterior
        # Re-order to rank order (R0 first, R1 second)
        post_ord  = _np_volhmm.array([post_raw[_vol1h_hmm_order[i]] for i in range(_n_vol1h)])
        r1_prob   = float(post_ord[1])
        # 10-step forward: posterior @ P^10, take R1 column
        r1_k10    = float(post_ord @ _vol1h_P10[:, 1])
        return (hard_rank, round(r1_prob, 4), round(r1_k10, 4))
    except Exception:
        return None
...
CSV_COLUMNS = [
    ...
    "markov_regime_daily",  # BTC 20-day rolling return regime label: Bull / Bear / Sideways [ACTIVE — sideways gate; intermittent fetch failures = blanks]
    ...
    "hmm_vol_state",     # hard Viterbi rank: 0=R0 low-vol, 1=R1 high-vol [SHADOW — gate after 100+ R1 obs]
    "hmm_r1_prob",       # soft posterior P(R1|data) — continuous 0-1; catches transitions ~5 bars early [SHADOW]
    "hmm_vol_k10",       # 10-step forward P(R1): posterior @ P^10 — regime persistence forecast [SHADOW]
...
def main() -> None:
    ...
    # Vol-regime HMM: compute once here so all three columns share one call.
