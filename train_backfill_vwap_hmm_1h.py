"""
Train VWAP multi-timeframe HMM for BTC HOURLY, mirroring
train_backfill_vwap_hmm_15m.py's methodology exactly (BIC state selection,
MCPT significance, week-by-week consistency, Kelly-style PnL simulation) --
but with windows re-scaled for an hourly contract's decision horizon.

The 15m model uses n=20-bar windows on 1m/5m/15m bars (20min/100min/5hr) --
context sized for a contract expiring in 15-60 minutes. For an hourly
contract, the SAME n=20-bar, ~5x-per-tier structure is preserved but shifted
up one granularity level: 1h/4h/1d bars instead of 1m/5m/15m.

Features (5 dims, all aligned to 1h bars):
  vwap_dist_1h  — (close - rolling_20bar_vwap_1h) / vwap * 100  (20-hour VWAP)
  vwap_dist_4h  — same on 4h bars (80-hour VWAP, ~3.3 days)
  vwap_dist_1d  — same on 1d bars (20-day VWAP)
  vwap_vel_1h   — 1h diff of vwap_dist_1h
  vwap_spread   — vwap_dist_1h - vwap_dist_1d  (cross-TF agreement)
"""
...
PARQUET = sorted(BASE.glob("data/binanceus_BTCUSDT_1m_1970-01-01_*.parquet"))[-1]
ARCH_PATH = BASE / "results" / "btc_scan_archive.csv"
STATES_OUT = BASE / "results" / "btc_vwap_hmm_states_1h.csv"  # sidecar, not modified by runner
MODEL_OUT = BASE / "models" / "hmm_vwap_mtf_btc_1h.pkl"
FEAT_COLS = ["vwap_dist_1h", "vwap_dist_4h", "vwap_dist_1d", "vwap_vel_1h", "vwap_spread"]
...
def build_feature_matrix(df1m: pd.DataFrame) -> pd.DataFrame:
    """Compute all 5 VWAP features aligned to 1h bar timestamps."""
    RESAMPLE_AGG = {"open": "first", "high": "max", "low": "min",
                    "close": "last", "volume": "sum"}

    df1h = df1m.resample("1h").agg(RESAMPLE_AGG).dropna()
    df4h = df1m.resample("4h").agg(RESAMPLE_AGG).dropna()
    df1d = df1m.resample("1d").agg(RESAMPLE_AGG).dropna()

    dist_1h = rolling_vwap_dist(df1h, 20)   # 20-hour VWAP
    dist_4h = rolling_vwap_dist(df4h, 20)   # 80-hour VWAP (~3.3 days)
    dist_1d = rolling_vwap_dist(df1d, 20)   # 20-day VWAP
    vel_1h = dist_1h.diff()                 # 1h momentum of VWAP divergence

    feat = pd.DataFrame(index=df1h.index)
    feat["vwap_dist_1h"] = dist_1h
    feat["vwap_dist_4h"] = dist_4h.reindex(df1h.index, method="ffill")
    feat["vwap_dist_1d"] = dist_1d.reindex(df1h.index, method="ffill")
    feat["vwap_vel_1h"] = vel_1h
...
# Split into continuous sequences (gap > 2h -> new seq)
ts = feat.index
gaps = pd.Series(ts).diff().dt.total_seconds().fillna(0).values
split_mask = gaps > 7200
...
# ── analysis ──────────────────────────────────────────────────────────────────
# btc_scan_archive.csv logs composite_p_up directly (not separate p_model_yes/no
# like the 15m archive) -- BTC hourly's REAL model applies regime-conditioning +
# p_up_v2 overrides on top of this, but composite_p_up vs p_market is a reasonable,
# disclosed simplification for "which side does the model lean" in this diagnostic.
...
for col in ["resolved_yes", "p_market", "composite_p_up"]:
    arch_res[col] = pd.to_numeric(arch_res[col], errors="coerce")
arch_res = arch_res.dropna(subset=["resolved_yes", "p_market", "composite_p_up", "vwap_hmm_state"])

arch_res["edge_yes"] = arch_res["composite_p_up"] - arch_res["p_market"]
arch_res["edge_no"] = (1 - arch_res["composite_p_up"]) - (1 - arch_res["p_market"])
...
for r in candidates:
    ...
    for wk in all_weeks:
        ...
        delta_wk = wr - base
        flag = "OK" if delta_wk * r["delta"] > 0 else "x"
...
def mcpt(df: pd.DataFrame, state: int, side: str) -> dict:
    ...
    return dict(state=state, side=side, n=(state_arr == state).sum(), obs_wr=obs_wr,
...
# ── PnL simulation (flat-fraction sizing, gate-effect diagnostic) ──────────────
# NOT a replica of BTC hourly's real regime-conditioned Kelly formula (too complex
# to faithfully reproduce here) -- flat proportional-to-edge sizing on a fixed
# bankroll, same simplification/disclosure pattern used throughout this session
# for gate-candidate PnL diagnostics (feedback_flat_bankroll_backtest).

print("\n" + "=" * 60)
print("PnL SIMULATION — flat-fraction sizing, $2.5k bankroll, diagnostic only")
print("=" * 60)
...
def flat_pnl(pm, p_model, side, resolved_yes):
    pm = float(pm)
    if side == "YES":
...
arch_dedup = (arch_res.sort_values("logged_at")
                       .groupby("contract_ticker", as_index=False).last())
print(f"Unique resolved contracts with state: {len(arch_dedup):,}")

def best_side_pnl(row):
    ey = float(row.composite_p_up) - float(row.p_market)
    en = float(row.p_market) - (1 - float(row.composite_p_up))
    if ey >= en and ey > MIN_EDGE:
        p = flat_pnl(row.p_market, row.composite_p_up, "YES", row.resolved_yes)
        return p, "YES"
    elif en > ey and en > MIN_EDGE:
        p = flat_pnl(row.p_market, 1 - row.composite_p_up, "NO", row.resolved_yes)
        return p, "NO"
    return None, None
...
if block_results:
    ...
    csr = sim_gate(taken, combo_mask, f"COMBO all {len(block_results)} blocks", baseline_pnl)
