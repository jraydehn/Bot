...
# ---------------------------------------------------------------------------
# p_up_v2 drift model — coherent single-K (2026-06-27 reform).
# K=0.50 applied symmetrically: p_no = 1 - p_yes always (no phantom edge).
# Brier sweep on 4,420 archive rows: K=0.3-0.5 optimal; K=1.40 was overfit.
K_PUP_V2_YES = 0.50
...
def run_scan(auth: Optional[KalshiAuth], bankroll: float, asset: str = "BTC",
    ...
    for c in contracts:
        ...
        # BTC: coherent single-K model (2026-06-27). p_no = 1 - p_yes always.
        #   p_yes = Φ(K×Φ⁻¹(p_up_v2)×√(τ/60) − z_strike), K=0.50 symmetric.
        #   edge  = p_yes − pm (positive→YES, negative→NO). No phantom edge.
        if asset == "BTC" and _p_up_v2_btc is not None:
            ...
            p_model_no  = 1.0 - p_model_yes   # coherent: p_yes + p_no = 1 always
        else:
            ...
            # BTC always coherent: p_model_no from LGBM is P(YES), invert it.
            if asset == "BTC":
        ...
        # BTC: coherent — edge_no = p_model_no-(1-pm) = (1-p_yes)-(1-pm) = pm-p_yes = -edge_yes.
        # BTC: coherent model — edge_no = -edge_yes.
        ...
        # [btc_5m_stoch_direction] Adapts the hourly sk1h<17→YES / sk1h>83→NO directional
        # gate to 5m stoch for 15m contracts. A 5m bounce/reversal resolves within the 15m
        # window, making 5m stoch the right timeframe signal (1h is too slow).
        # sim 6wk: sk5m<17→YES WR=75.4%, +11.1pp; sk5m>83→NO WR=60.2%, +10.0pp (MCPT p≤0.05).
        # Direction override sets synthetic edge floor (0.05) when coherent model disagrees,
        # matching how the hourly non-coherent model independently supports each direction.
        # Quality blocks from rescue search: chg_4h for YES; consec_dir/regime_z for NO.
        # Backup: paper_trade_runner_15m_pre_5m_stoch_zones_20260629.py
        if asset.upper() == "BTC":
            try:
                _sk5m_sd = float(sig.get("stoch_k_5m") or 50.0)
            except (TypeError, ValueError):
                _sk5m_sd = 50.0

            if _sk5m_sd < 17.0 and p_market >= 0.35:
                # Oversold → prefer YES (bounce signal). Minimum pm=0.35 avoids deep-OTM
                # where a 15m window is too short for price to recover to the strike.
                if best_side != "yes":
                    print(f"    [btc_5m_stoch_dir] FLIP {best_side.upper()}→YES {ticker} — "
                          f"sk5m={_sk5m_sd:.1f}<17 + pm={p_market:.3f}>=0.35 (oversold bounce, like hourly sk1h<17)")
                else:
                    print(f"    [btc_5m_stoch_dir] CONFIRM YES {ticker} — "
                          f"sk5m={_sk5m_sd:.1f}<17 (model+stoch agree)")
                best_side = "yes"
                best_edge = max(edge_yes, 0.05)
                p_model   = p_model_yes
                # Quality block: if 4h has no down context, bounce is weaker
                # Rescue: strong futures longs, extreme fear, or low vol confirm bounce
                try:
                    _chg4h_sd = float(sig.get("chg_4h") or 0.0)
                except (TypeError, ValueError):
                    _chg4h_sd = 0.0
                _rat4h_sd = (_cg.futures_ratio_4h if _cg else float("nan"))
                _fg_sd    = (_cg.fg_value         if _cg else float("nan"))
                try:
                    _vr_sd = float(sig.get("vol_ratio") or 1.0)
                except (TypeError, ValueError):
                    _vr_sd = 1.0
                if _chg4h_sd > -0.045:
                    _res_sd = (
                        (not math.isnan(_rat4h_sd) and _rat4h_sd > 1.037)
                        or (not math.isnan(_fg_sd)  and _fg_sd  <= 24.0)
                        or (_vr_sd <= 0.36)
                    )
                    if _res_sd:
                        print(f"    [btc_5m_stoch_dir] RESCUE YES {ticker} — "
                              f"chg_4h={_chg4h_sd:+.3f}>-0.045 rescued by "
                              f"ratio4h={_rat4h_sd:.3f}/fg={_fg_sd:.0f}/vr={_vr_sd:.3f}")
                    else:
                        print(f"    [btc_5m_stoch_dir] BLOCK YES {ticker} — "
                              f"sk5m={_sk5m_sd:.1f}<17 but chg_4h={_chg4h_sd:+.3f}>-0.045 "
                              f"(no 4h down context, WR=56.7%; no rescue)")
                        evaluated.append((best_edge, best_side, c, p_model, offset_pct))
                        continue

            elif _sk5m_sd > 83.0:
                # Overbought → prefer NO (reversal signal, like hourly sk1h>83→NO).
                if best_side != "no":
                    print(f"    [btc_5m_stoch_dir] FLIP {best_side.upper()}→NO {ticker} — "
                          f"sk5m={_sk5m_sd:.1f}>83 (overbought reversal, like hourly sk1h>83)")
                else:
                    print(f"    [btc_5m_stoch_dir] CONFIRM NO {ticker} — "
                          f"sk5m={_sk5m_sd:.1f}>83 (model+stoch agree)")
                best_side = "no"
                best_edge = max(edge_no, 0.05)
                p_model   = p_model_no
                # Quality block: consecutive up bars = trend continuation, NO loses
                try:
                    _cd15_sd = float(sig.get("consec_dir_15m") or 0.0)
                except (TypeError, ValueError):
                    _cd15_sd = 0.0
                if _cd15_sd in (1.0, 2.0):
                    print(f"    [btc_5m_stoch_dir] BLOCK NO {ticker} — "
                          f"sk5m={_sk5m_sd:.1f}>83 but consec_dir_15m={_cd15_sd:.0f}∈{{1,2}} "
                          f"(up-streak, WR=33-40%, -$242 on sim)")
                    evaluated.append((best_edge, best_side, c, p_model, offset_pct))
                    continue
                # Quality block: deeply negative regime drift weakens the overbought signal
                try:
                    _rz_sd = float(sig.get("regime_z") or 0.0)
                except (TypeError, ValueError):
                    _rz_sd = 0.0
                try:
                    _uw_sd = float(sig.get("upper_wick_15m") or 0.0)
                except (TypeError, ValueError):
                    _uw_sd = 0.0
                if _rz_sd <= -0.23:
                    _res_sd = (offset_pct > -0.003) or (_uw_sd > 0.115)
                    if _res_sd:
                        print(f"    [btc_5m_stoch_dir] RESCUE NO {ticker} — "
                              f"regime_z={_rz_sd:.3f}<=-0.23 rescued by "
                              f"offset={offset_pct:+.4f}/uw15m={_uw_sd:.3f}")
                    else:
                        print(f"    [btc_5m_stoch_dir] BLOCK NO {ticker} — "
                              f"sk5m={_sk5m_sd:.1f}>83 but regime_z={_rz_sd:.3f}<=-0.23 "
                              f"(negative drift kills reversal signal; no rescue)")
                        evaluated.append((best_edge, best_side, c, p_model, offset_pct))
                        continue

        # [coherent_flip_guard] All assets: after any flip gate, best_edge<0 means the
        # flip produced the losing side's edge. Coherent model can't recover from this — block.
        if best_edge < 0:
            print(f"    [coherent_guard] BLOCK {ticker} — flip rejected "
                  f"(edge={best_edge:+.3f}, coherent model)")
        ...
        if asset.upper() == "BTC" and best_side == "no":
            ...
            _c5 = sig.get("chg_5m")
            try:
                if _s5 is not None and _c5 is not None and 76.0 < float(_s5) <= 83.0 and float(_c5) > 0.0:
    ...
    # SOL YES Kelly dampener: WR=53.5% vs breakeven 57.3% (avg loss -$44.60 > avg win +$33.24).
    # Structural payout asymmetry — 0.10x reduces YES PnL from -$1,636 → -$164 (+$1,472 improvement).
    # MCPT confirms genuine negative edge (p=0.047); 3/7 weeks positive at 0.10x (preserves upside).
    # Implemented 2026-06-25. Backup paper_trade_runner_15m_pre_kalman_fix_20260625.py
    # Revert: remove this block.
    if asset == "SOL" and side == "yes":
        _undampened_sol = kelly.bet_amount
        kelly.bet_amount = round(kelly.bet_amount * 0.10, 2)
        print(f"    [sol_yes_kelly_damp] SOL YES ×0.10 "
              f"(${_undampened_sol:.2f} → ${kelly.bet_amount:.2f})")
