...
def compute_p_model_15m(spot: float, floor_strike: float,
    ...
    ETH: p5_up=(bp+stoch/100)/2 linear drift (scale=2.0) + body contra z-drift (>=0.7); stoch<44 YES gate.
    BTC: tanh bp drift (scale=3.0) + body trend step delta; no stoch gate.
    SOL: p_market base + contra-bp drift (scale=1.0) — mean-reversion; low bp→YES edge, high bp→NO edge.
    ...
    if asset.upper() == "ETH":
        # p5_up: blend bp and stoch for smoother 5m directional estimate; linear mapping avoids norm.ppf tail blow-up
        bp_raw = sig.get("bp_5m", 0.5)
        stoch5 = sig.get("stoch_k_5m", 50.0) / 100.0
        p5_up  = (bp_raw + stoch5) / 2.0
        z_bp   = (p5_up - 0.5) * 2.0 * math.sqrt(tau_min / 5.0)
        # body contra-drift: large bars (>=0.7) signal exhaustion — ETH mean-reverts
        ...
        # tanh bp drift — same scale as linear for moderate bp (tanh(x)≈x near 0),
        # saturates smoothly at extremes to prevent blow-up when bp≈0 or bp≈1.
        bp_raw = sig.get("bp_5m", 0.5)
        z_bp   = math.tanh((bp_raw - 0.5) * 3.0) * math.sqrt(tau_min / 5.0)
        p_base = float(norm.cdf(z_base + z_bp))
        # body trend step delta: large bars signal continuation for BTC
        delta  = 0.0
        body15 = sig.get("body_15m", 0.0)
        dir15  = sig.get("dir_15m", 0)
        if body15 >= 0.50:
        ...
        # SOL: p_market base + contra-bp drift (mean-reversion at 15m).
        # Low bp (sellers dominated last bar) → price depressed → fade back up → YES edge.
        # High bp (buyers dominated) → price extended → fade back down → NO edge.
        # scale=1.0 (conservative vs BTC 3.0); fallback to log-normal if p_market unavailable.
        pm_clamped = max(0.03, min(0.97, p_market))
        z_base_sol = float(norm.ppf(pm_clamped))
        bp     = sig.get("bp_5m", 0.5)
        z_drift = -math.tanh((bp - 0.5) * 1.0) * math.sqrt(tau_min / 5.0)
        p_base = float(norm.cdf(z_base_sol + z_drift))
        delta  = 0.0
...
def run_scan(auth: Optional[KalshiAuth], bankroll: float, asset: str = "BTC",
    ...
    for c in contracts:
        ...
        # ETH YES ema gate: block YES when EMA is bearish (confirmed downtrend).
        # Full-history (93 resolved): ETH YES + ema=-1 → WR=40.9% (below breakeven).
        # Rescue (stoch<10 + big flush) dropped — full data shows 5L/1W at stoch<10, no edge.
        if asset.upper() == "ETH" and best_side == "yes":
            if sig.get("ema_bias", 0) == -1:
                best_side, best_edge = "no", edge_no
