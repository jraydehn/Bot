...
def main() -> None:
    ...
    if auth is not None:
        ...
        for c in ladder:
            ...
            _gbdt_promoted = False  # True if GBDT promoted this from no_trade → trade
            s_k       = c["floor_strike"]
            pm        = c["p_market"]
            ...
            if args.asset == "BTC" and _composite_computed and sigma_tau_c > 0:
                ...
                # BTC LGBM per-contract inference (gate + promotion + scan archive logging)
                _gbdt_feats_c = {
                    "offset_pct":         offset_c,
                    "p_market":           pm,
                    "tau_minutes":        tau_c,
                    "side_enc":           0.5,
                    "composite_p_up":     _comp_p_up_c if _comp_p_up_c is not None else float("nan"),
                    "composite_trend":    float(_active_trend),
                    "composite_rev":      float(_active_rev),
                ...
                # Ghost log: record every evaluated BTC contract for richer LGBM training
                try:
                    import btc_scan_archive as _bsa
                    _bsa.log_scan_row(
                        ticker=c["ticker"], close_ts=c["close_time"],
                        spot=spot, strike=s_k, p_market=pm, tau_minutes=tau_c,
                        features=_gbdt_feats_c, p_gbdt=_p_gbdt_c, now_utc=now_utc,
                    )
                except Exception:
                    pass
                ...
                if _dec_yes is not None and _dec_yes.decision == "trade" and _dec_no is not None and _dec_no.decision == "trade":
                    ...
                    dec_c = _dec_no
                    # GBDT promotion: if GBDT sees strong edge on no_trade, try to upgrade
                    if dec_c.decision == "no_trade" and _p_gbdt_c is not None:
                        _gbdt_ye = _p_gbdt_c - pm
                        _gbdt_ne = pm - _p_gbdt_c
                        _gbdt_be = max(_gbdt_ye, _gbdt_ne)
                        _gbdt_ps = "yes" if _gbdt_ye >= _gbdt_ne else "no"
                        if _gbdt_be > 0.08:
                            _gbdt_pmod  = _p_gbdt_c if _gbdt_ps == "yes" else (1.0 - _p_gbdt_c)
                            _gbdt_pmref = c["ask"] if _gbdt_ps == "yes" else c["bid"]
                            _dec_promo  = evaluate_trade(
                                struct.structure_bias, confirm.confirmation_bias,
                                _gbdt_pmod, _gbdt_pmref, args.bankroll,
                                confirmation_score=_cscore, no_score=_nscore,
                                obi_score=confirm.obi_score, vol_score=confirm.vol_score,
                                ema_alignment=_ema_align, asset=args.asset,
                                composite_active=_composite_active, composite_p_up=_comp_p_up,
                                offset_pct=offset_c, force_side=_gbdt_ps)
                            if _dec_promo is not None and _dec_promo.decision == "trade":
                                print(f"  [btc_gbdt_promote] PROMOTE {_gbdt_ps.upper()} "
                                      f"{c['ticker']} — p_gbdt={_p_gbdt_c:.3f} pm={pm:.3f} "
                                      f"gbdt_edge={_gbdt_be:+.3f}>0.08 (base model had no edge)")
                                dec_c = _dec_promo
                                _gbdt_promoted = True
                else:
            ...
            # [btc_gbdt_gate] Block BTC when GBDT disagrees with the decided side by >0.03.
            # Test-set P&L (May 5-18, n=309): GBDT-agrees +$2,477; base-only -$3,040 (2026-05-17).
            # Threshold -0.03 avoids blocking near-neutral disagreements that add noise.
            # Not applied to GBDT-promoted trades (those were selected BY the GBDT).
            if (args.asset == "BTC" and dec_c.decision == "trade"
                    and _p_gbdt_c is not None and not _gbdt_promoted):
                _gbdt_edge_side = (_p_gbdt_c - pm) if dec_c.side == "yes" else (pm - _p_gbdt_c)
                if _gbdt_edge_side < -0.03:
                    print(f"  [btc_gbdt_gate] BLOCK {dec_c.side.upper()} {c['ticker']} — "
                          f"p_gbdt={_p_gbdt_c:.3f} pm={pm:.3f} "
                          f"gbdt_edge={_gbdt_edge_side:+.3f}<-0.03 (GBDT disagrees)")
                    _log_block("btc_gbdt_gate")
                    continue
        ...
        if loop_count % 5 == 0:
            ...
            if _loop_is_live_mode:
                ...
                if _live_auth:
                    ...
                    try:
                        import btc_scan_archive as _bsa
                        _bsa.fill_scan_outcomes(_live_auth)
                    except Exception:
                        pass
            else:
