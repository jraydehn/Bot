...
def compute_p_model_15m(spot: float, floor_strike: float,
    ...
    BTC: linear bp drift (scale=3.0) + body trend step delta; no stoch gate.
    SOL: body trend z-drift (>=0.5, dir15 momentum); stoch 40-80 YES gate in run_scan.
    ...
    if asset.upper() == "ETH":
        ...
        # SOL: body trend z-drift — large bar direction predicts continuation; bp adds noise, removed
        body15 = sig.get("body_15m", 0.0)
        dir15  = sig.get("dir_15m", 0)
        if body15 >= 0.5:
            z_body = float(norm.ppf(max(0.05, min(0.95, body15)))) * dir15 * math.sqrt(tau_min / 5.0)
            ...
            z_body = 0.0
        p_base = float(norm.cdf(z_base + z_body))
        delta  = 0.0
...
def run_scan(auth: Optional[KalshiAuth], bankroll: float, asset: str = "BTC",
    ...
    for c in contracts:
        ...
        # SOL YES gate: block YES when bp<0.5 (body+bp must agree on direction)
        # Rescue: allow YES through when stoch>=80 (extreme overbought, tentative — n=4)
        if asset.upper() == "SOL" and best_side == "yes":
            bp    = sig.get("bp_5m", 0.5)
            stoch = sig.get("stoch_k_5m", 50.0)
            if bp < 0.5 and stoch < 80:
                best_side, best_edge = "no", edge_no
