...
def evaluate_trade(
    ...
    # Calibration is now side-specific:
    #
    ...
    if asset == "BTC" and ema_alignment == "bullish" and side == "yes":
        reasons.append(
            f"Gate EMA-Dir FAILED: ema=bullish+YES — continuation bet, 8.1% win rate "
            f"(n=2,911 backtest bars). 1h contracts do not sustain momentum after bullish EMA."
        )
    ...
    if asset == "BTC":
        reasons.append(
            ...
            f"bullish+YES block is the only EMA-Dir restriction."
        )
    ...
    # --- Gate NS: no_score ≥ 1 for BTC NO bets ---
    # Paper trade analysis (n=282 resolved BTC trades, 2026-04-07):
    #   no_score=0 NO: 55.3% win — below break-even at p_market 0.20–0.35 (need 65–80%)
    #   no_score=1 NO: 73.4% win — profitable at every p_market bin tested
    #   no_score=2 NO: 86.8% win
    # The no_score=0 group fails because it includes "confused market" bars where
    # confirmation indicators are mixed/neutral — exactly the regime where a 1h NO
    # bet on BTC has no directional edge. Gate PM alone is insufficient; no_score
    # provides the required momentum confirmation.
    # ETH/SOL: minimum offset filters applied in paper_trade_runner.py instead
    #   (ETH NO ≥ 0.20% offset, SOL NO ≥ 0.50% offset — 2026-04-07 real Kalshi pricing analysis)
    # Revert: copy decision_v3.py → decision.py
    if asset == "BTC" and side == "no" and no_score < 1:
    ...
    # Formula:
    #   YES: rr = p_market / (1 - p_market)  — cost per dollar of potential payout
    #        Lower rr = cheaper contract = better reward:risk for the bettor.
    #        No lower bound applied: a YES at pm=0.10 risks $0.10 to win $0.90 (9:1
    #        reward:risk) — that is favorable, not unfavorable.
    #
    #   NO:  rr = (1 - p_market) / p_market  — decreases with p_market
    #        Lower bound (NO only): rr < 0.33 blocks near-ATM NO (pm > 0.75) where
    #        you risk a lot to win little.
    #        Upper bound (NO only): rr > 4.0 blocks cheap NO bets (pm < 0.20) where
    #        payout is tiny relative to risk — historically negative PnL despite high
    #        win rate due to outsized losses.
    RR_MAX_NO = 4.0
    RR_MIN_NO = 0.33
    rr = p_market / (1 - p_market) if side == "yes" else (1 - p_market) / p_market
    rr_fail = side == "no" and (rr < RR_MIN_NO or rr > RR_MAX_NO)
    if rr_fail:
