...
MONITOR_COLUMNS = [
    ...
    "prev_mid",            # mid from previous monitoring cycle (for mid_delta)
    "appreciation_x",      # (1-mid)/(1-entry_pm) for NO; mid/entry_pm for YES
    ...
    "dist_to_strike",      # (strike-spot)/spot; positive = below strike (ITM for NO)
    "btc_1m_chg",          # last 1m spot return
    "btc_5m_chg",          # last 5m spot return
...
PM_LOCK_THRESHOLD      = 0.98   # p_market level at which locking in beats risking the remaining 2¢
...
# Mid-delta reversal signal: market is repricing fast while NO position is profitable
MID_DELTA_APPX_THRESH  = 1.5    # minimum no_appx to arm the signal
MID_DELTA_THRESH       = 0.04   # YES mid must rise this much vs previous cycle

# Velocity-approach signal: BTC moving fast toward strike with little cushion left
VELOCITY_DIST_THRESH   = 0.005  # BTC within 0.5% of strike
VELOCITY_1M_THRESH     = 0.002  # 1m BTC rally ≥ 0.2% toward strike


# Per-ticker mid cache for mid_delta computation — primed each evaluation cycle
_prev_mid_cache: dict = {}
...
def evaluate_open_positions(
    ...
    # Asset-level 1m/5m momentum — computed once per evaluation call
    _asset_1m_chg = None
    _asset_5m_chg = None
    if df_1m is not None and len(df_1m) >= 6:
        try:
            cl = df_1m["close"]
            _asset_1m_chg = round(float(cl.iloc[-1] / cl.iloc[-2] - 1), 6)
            _asset_5m_chg = round(float(cl.iloc[-1] / cl.iloc[-6] - 1), 6)
        except Exception:
            pass
    ...
    for pos in positions:
        ...
        # Correct appreciation for each side:
        # NO: (1-mid)/(1-entry_pm) — how much of the NO value has been captured
        # YES: mid/entry_pm — how much YES has appreciated
        if side == "no":
            appreciation_x = round((1.0 - mid) / (1.0 - entry_p_market), 3) if entry_p_market < 1.0 else 0.0
        else:
        ...
        # Position-level spot signals
        dist_to_strike = round((strike - spot) / spot, 6) if spot > 0 and strike > 0 else None
        prev_mid_val   = _prev_mid_cache.get(ticker)
        _mid_delta     = round(mid - prev_mid_val, 4) if prev_mid_val is not None else None
        ...
        # Signal 4: profit_lock — 1.5× gain AND near-resolution price AND short tau
        # Fixed for NO: use (1-mid) >= 0.85 instead of mid >= 0.85
        _near_resolution = (
            (side == "yes" and mid >= PROFIT_LOCK_MID) or
            (side == "no"  and (1.0 - mid) >= PROFIT_LOCK_MID)
        )
        _profit_lock = (_tau_valid
                        and appreciation_x >= PROFIT_LOCK_APPX
                        and _near_resolution
                        and tau_remaining < PROFIT_LOCK_TAU)

        # Signal 5: mid_delta reversal — YES mid rising fast while NO position is profitable.
        # Detects the market repricing a reversal before it fully materialises.
        # Backtest (189 resolved NO, n=20 fires): +$165 net vs hold; every exit positive.
        _sell_mid_delta = (side == "no"
                           and _mid_delta is not None
                           and appreciation_x >= MID_DELTA_APPX_THRESH
                           and _mid_delta > MID_DELTA_THRESH)

        # Signal 6: velocity approach — BTC already close to strike and rallying hard.
        # Fires when the physical gap between BTC and strike is small enough that
        # current 1m momentum could breach the strike before expiry.
        _sell_velocity = (side == "no"
                          and dist_to_strike is not None
                          and _asset_1m_chg is not None
                          and 0 < dist_to_strike < VELOCITY_DIST_THRESH
                          and _asset_1m_chg > VELOCITY_1M_THRESH)

        would_sell  = (_sell_edge or _sell_pm or _stop_loss or _profit_lock
                       or _sell_mid_delta or _sell_velocity)
        sell_reason = ",".join(filter(None, [
            ...
            "mid_reversal" if _sell_mid_delta   else "",
            "velocity"     if _sell_velocity    else "",
        ]))
        ...
        row = {
            ...
            "prev_mid":        round(prev_mid_val, 4) if prev_mid_val is not None else "",
            ...
            "dist_to_strike":  round(dist_to_strike, 6) if dist_to_strike is not None else "",
            "btc_1m_chg":      round(_asset_1m_chg, 6) if _asset_1m_chg is not None else "",
            "btc_5m_chg":      round(_asset_5m_chg, 6) if _asset_5m_chg is not None else "",
        ...
        _prev_mid_cache[ticker] = mid  # prime for next cycle
        new_rows.append(row)
        ...
        _dist_str = f"{dist_to_strike*100:+.2f}%" if dist_to_strike is not None else "n/a"
        _1m_str   = f"{_asset_1m_chg*100:+.3f}%" if _asset_1m_chg  is not None else "n/a"
        _dlt_str  = f"{_mid_delta:+.3f}" if _mid_delta is not None else "n/a"
        print(
            f"  [pos_monitor] {ticker}  {appreciation_x:.2f}×  "
            f"mid={mid:.3f}(Δ{_dlt_str})  edge={_edg_str}  "
            f"dist={_dist_str}  1m={_1m_str}  "
