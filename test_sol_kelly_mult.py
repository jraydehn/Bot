"""test_sol_kelly_mult.py (scratch) — SOL hourly Kelly multiplier sweep (net PnL).
SOL currently uses 0.50 (decision.py else-branch). Reconstruct p_yes via real
score_to_p_model(asset='SOL', drift 0.20); sweep multiplier; per-expiry cap 2; two halves."""
...
a=pd.read_csv("results/sol_scan_archive.csv",low_memory=False)
...
def pyes(row):
    ...
    try: return cs.score_to_p_model(int(row.composite_trend),int(row.composite_rev),row.spot,row.strike,sig,asset="SOL",p_up_override=row.composite_p_up)
...
def bt(df,mult):
    d=df[df.edge>MIN_EDGE]; traded=set(); ec={}; pnl=0.0; n=0; capped=0; pe=[]
...
print(f"SOL archive contracts: {len(a)}  side mix: {a.side.value_counts().to_dict()}")
