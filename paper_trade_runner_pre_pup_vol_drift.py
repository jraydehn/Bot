    ...
    if auth is not None:
            ...
            try:
                ...
                try:
                    ...
                    try:
                        ...
                        _arima_forecast_btc = float("nan")
                    print(f"  [drift] mu6={_mu_6h_btc:.6f} mu12={_mu_12h_btc:.6f} "
                          f"mu24={_mu_24h_btc:.6f} regime_z={_regime_z_btc:.3f} "
                          f"garch_ve={_garch_ve_btc:.6f} arima={_arima_forecast_btc:.7f}")
        ...
        for c in ladder:
            ...
            if _composite_computed:
                ...
                # YES spec: mu6h + mu24h + regime_z + composite_trend/5 × 0.15
                if args.asset == "BTC" and sigma_tau_c > 0:
                    _sq = math.sqrt(tau_c / 60.0)
                    _z_drift_6h = (
                        (_mu_6h_btc + _mu_24h_btc) * (tau_c / 60.0) / sigma_tau_c
                        + _regime_z_btc * _sq
                        + (float(_active_trend) / 5.0) * 0.15 * _sq
                    )
                else:
                    _z_drift_6h = 0.0
                ...
                if args.asset == "BTC" and _composite_computed:
                    ...
                    if _p_up_v2 is not None:
                        ...
                        # p_up_v2 is used as a NO-side z_drift input — not as p_up override.
            ...
            if args.asset == "BTC" and _composite_computed and sigma_tau_c > 0:
                # NO spec: mu6h + mu12h + mu24h + regime_z + p_up_v2; GARCH σ override
                ...
                _z_drift_no = 0.0
                if _sigma_tau_no > 0:
                    _z_drift_no += (_mu_6h_btc + _mu_12h_btc + _mu_24h_btc) * (tau_c / 60.0) / _sigma_tau_no
                    _z_drift_no += _regime_z_btc * _sq_no
                    if _p_up_v2 is not None:
                        from scipy.stats import norm as _norm_no
                        _z_drift_no += _norm_no.ppf(max(0.01, min(0.99, _p_up_v2))) * 1.14 * _sq_no
