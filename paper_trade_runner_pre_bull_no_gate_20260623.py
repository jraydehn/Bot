...
def main() -> None:
    ...
    if auth is not None:
        ...
        for c in ladder:
            ...
            # [btc_strong_bull_no_gate] Block BTC NO when composite_trend >= 3 (strong bull).
            # Archive (real Kalshi pm, 718 expiries): T1 edge=-2.5% in strong_bull (n=122);
            # T3 MCPT p=0.040 (significantly bad). Even the primary NO pick is below breakeven
            # in sustained uptrend. bull_rally_no_gate catches active hour-rallies; this catches
            # the broader sustained trend state. Backup: pre_expiry_cap_adaptive_20260623.py
            if (args.asset == "BTC" and dec_c.decision == "trade"
                    and dec_c.side == "no"
                    and _active_trend >= 3):
                print(f"  [btc_strong_bull_no_gate] BLOCK NO {c['ticker']} — "
                      f"trend={_active_trend} >= 3 (strong bull: T1 edge=-2.5% archive, "
                      f"T3 MCPT p=0.040)")
