...
def run_scan(auth: Optional[KalshiAuth], bankroll: float, asset: str = "BTC",
    ...
    for c in contracts:
        ...
        # [btc_15m_no_overbought_gate] Block NO when stoch_k_15m>=80 AND ema_bias=+1.
        # Scan archive (2026-05-27, n=36 NO bets): WR=5.6%, BE=10.2%, -12.8pp, PnL=-$133.
        # dir=+1 sub-slice (n=21): WR=0.0%, -$158 — zero wins, no rescue found.
        # Rescue: bp_15m<0.35 AND dir=-1 (n=12): WR=16.7%, +6.1pp vs BE, +$44 — allow.
        # Logic: overbought 15m stoch + bullish EMA = shorting pure momentum; rescue when
        # most recent 15m bar was a down-candle closing near its low (local exhaustion).
        if asset.upper() == "BTC" and best_side == "no":
            _sk15_no = float(sig.get("stoch_k_15m", 50.0) or 50.0)
            _ema_no  = float(sig.get("ema_bias", 0.0) or 0.0)
            if _sk15_no >= 80.0 and _ema_no == 1.0:
                _bp15_no  = float(sig.get("bp_15m", 0.5) or 0.5)
                _dir15_no = sig.get("dir_15m", 0)
                _no_ovb_rescue = (_bp15_no < 0.35 and _dir15_no == -1)
                if not _no_ovb_rescue:
                    print(f"    [btc_15m_no_overbought_gate] BLOCK NO → skip {ticker} — "
                          f"stoch_k_15m={_sk15_no:.1f}>=80, ema_bias=+1, "
                          f"dir={_dir15_no}, bp_15m={_bp15_no:.2f} "
                          f"(momentum overbought, WR=5.6% archive, -$133)")
                    evaluated.append((best_edge, best_side, c, p_model, offset_pct))
                    continue
                else:
                    print(f"    [btc_15m_no_overbought_gate] RESCUE NO {ticker} — "
                          f"stoch_k_15m={_sk15_no:.1f}>=80+ema=+1 but "
                          f"dir=-1+bp_15m={_bp15_no:.2f}<0.35 "
                          f"(down-candle near low, local exhaustion, WR=16.7%)")
