...
def run_asset(asset: str, bankroll: float) -> None:
...
def main() -> None:
    ...
    assets = [
        (a, br) for a, br in [
            ("BTC", args.btc_bankroll),
            ("ETH", args.eth_bankroll),
            ("SOL", args.sol_bankroll),
        ] if a not in _skip
    ]

    print("=" * 60)
    print("  15M PAPER TRADERS — BTC / ETH / SOL")
    print("=" * 60)
    for asset, br in assets:
        print(f"  {asset}: bankroll=${br:,.0f}")
    print()

    threads = []
    for asset, bankroll in assets:
        t = threading.Thread(target=run_asset, args=(asset, bankroll), daemon=True)
        t.start()
        threads.append(t)
