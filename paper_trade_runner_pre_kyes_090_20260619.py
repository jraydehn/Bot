        ...
        for c in ladder:
            ...
            if _composite_computed:
                ...
                if args.asset == "BTC" and _composite_computed:
                    ...
                    # YES drift: norm.ppf(comp_p_up_c) × rvol_inv × k=1.40 × √(τ/60)
                    if sigma_tau_c > 0:
                        from scipy.stats import norm as _norm_pup
                        _z_drift_6h = float(_norm_pup.ppf(max(0.01, min(0.99, _comp_p_up_c)))) * _rvol_inv_btc * 1.40 * math.sqrt(tau_c / 60.0)
            ...
            # [expiry_cap2 2026-06-18] 3→2: same-expiry NO bets are highly correlated (64% of
            # multi-bet expiries resolve all-same-outcome). Independently-sized correlated bets
            # multiply variance for the same per-expiry edge. Month archive (626 expiries): the
            # 3rd bet loses $2.4-3.4k vs cap-2; cap-2 best total $, cap-1 best Sharpe. Edge
            # concentrates in the single deepest-OTM NO. Backup: paper_trade_runner_pre_expiry_cap2_20260618.py
