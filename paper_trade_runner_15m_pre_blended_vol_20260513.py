...
def _compute_1h_drift(sig: dict, tau_min: float) -> float:
    """
    Composite 1h directional drift from the 7 signals most correlated with
    15m forward price direction (from 82k-bar sweep, 2026-05-12).

    Each component normalized to ~[-1, +1] via tanh. Weighted by r magnitude.
    Sum range: [-1.45, +1.45]. Final scale: 0.25 * sqrt(tau/5) keeps max
    z_drift ≈ ±0.61 at tau=14min (shifts p_model ~15pp at most).
    """
    chg1h  = sig.get("chg_1h",              0.0)   # % — r=0.35
    bp1h   = sig.get("bp_1h",               0.5)   # 0-1 — r=0.26
    streak = sig.get("consec_dir_1h",        0.0)   # ±5 — r=0.20
    sk1h   = sig.get("stoch_k_1h",          50.0)  # 0-100 — r=0.15
    dc_brk = sig.get("donchian_breakout_1h", 0.0)  # ±1 — r=0.18
    eng1h  = sig.get("engulfing_1h",         0.0)  # ±1 — r=0.16
    sc1h   = sig.get("stoch_cross_1h",       0.0)  # ±1 — r=0.15

    composite = (
        0.35 * math.tanh(chg1h  / 1.0) +           # 1% move → tanh≈0.76
        0.26 * math.tanh((bp1h - 0.5) * 4.0) +     # bp=0.8  → tanh≈0.83
        0.20 * math.tanh(streak / 2.5) +            # streak=3 → tanh≈0.84
        0.15 * math.tanh((sk1h - 50.0) / 25.0) +   # sk=80   → tanh≈0.76
        0.18 * float(dc_brk) +
        0.16 * float(eng1h) +
        0.15 * float(sc1h)
    )
    # composite ∈ [-1.45, +1.45]; scale to z, modulated by remaining time
    SCALE = 0.25
...
def compute_p_model_15m(spot: float, floor_strike: float,
    ...
    All assets: p_market base (norm.ppf(p_market)) + 1h composite drift.
    The market already prices in spot/strike distance; the drift adds the
    directional signal from 1h momentum (r=0.15–0.35 across all assets).

    ETH: keeps validated 5m gates (stoch<44, ema=-1) as YES blockers.
    SOL: adds small contra-bp micro-adjustment (mean-reversion at 15m confirmed).
    All: tau confidence decay (tau/5)^2 applied to edge qualification below 5 min.
    """
    if tau_min <= 0.5 or spot <= 0 or floor_strike <= 0:
        return 0.5

    # p_market base: market already prices spot/strike; drift adds directional info
    pm_clamped = max(0.03, min(0.97, p_market))
    z_base = float(norm.ppf(pm_clamped))

    # Primary driver: 1h composite drift (same for all assets)
    z_1h = _compute_1h_drift(sig, tau_min)

    # Asset-specific micro-adjustment (small, secondary)
    z_micro = 0.0
    if asset.upper() == "SOL":
        # SOL mean-reverts at 15m (dir_15m r≈-0.007): add a small contra-bp nudge
        bp = sig.get("bp_5m", 0.5)
        z_micro = -math.tanh((bp - 0.5) * 0.5) * 0.10 * math.sqrt(tau_min / 5.0)

    p_model = float(norm.cdf(z_base + z_1h + z_micro))
