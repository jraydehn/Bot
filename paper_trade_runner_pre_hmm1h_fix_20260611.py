...
def _ms_hmm_state(live_1m: "pd.DataFrame", asset: str) -> "tuple[int, float] | None":
    """Return (state, state_prob) from live 1m data using the per-asset microstructure HMM.
    ...
    if pkg is None or live_1m is None:
        return None
    try:
...
def _vd_hmm_state(live_1m: "pd.DataFrame", asset: str) -> "tuple[int, float] | None":
    """Return (state, state_prob) from live 1m data using the per-asset vol+direction HMM.
...
def main() -> None:
    ...
    if auth is not None:
        ...
        # Microstructure HMM state — computed once per scan cycle for all assets.
        _hmm_ms_result = _ms_hmm_state(live_1m, args.asset) if live_1m is not None else None
        ...
        # Vol+Direction HMM state — computed once per scan cycle from live 1h price bars.
        _hmm_vd_result = _vd_hmm_state(live_1m, args.asset) if live_1m is not None else None
        ...
        for c in ladder:
            ...
            # Block YES when ADX ∈ [30,40) AND composite_trend < 0 AND pm ∈ [0.70, 0.85).
            # 2026-06-07 reform: narrowed from full pm range to [0.70,0.85) — below 0.70 failed
            # MCPT (p=0.068); [0.70,0.85) validates at z=+6.95, p=0.000, n=492, WR=61.6% vs bkev=78.1%.
            # Rescue: rvol_1h>=0.40 (WR=86.9%, +8.7pp) OR hmm_vol_state==1 (WR=92.4%, +13.7pp).
            # Logic: near-ITM YES in moderate trend with bearish composite = priced for upside that
            # signals don't support. High vol or R1 regime overrides (rally is real).
            # Backup: paper_trade_runner_pre_remove_near_itm_20260607.py
            if (args.asset == "BTC" and _composite_computed and not _otm_yes_blocked
                ...
                _adx_rvol = _rvol_1h if not math.isnan(_rvol_1h) else 0.0
                _adx_r1   = (getattr(confirm, "hmm_vol_state", None) == 1)
                _adx_rescued = _adx_rvol >= 0.40 or _adx_r1
                if _adx_rescued:
                    print(f"  [adx_mid_ct_neg_yes_gate] RESCUE YES {c['ticker']} — "
                          f"adx={_adx_mid:.1f} ct={_active_trend} pm={pm:.3f} "
                          f"rvol={_adx_rvol:.2f} R1={_adx_r1} (high vol/regime rescue)")
                else:
                    _otm_yes_blocked = True
                    _otm_yes_block_gate = "adx_mid_ct_neg_yes_gate"
                    print(f"  [adx_mid_ct_neg_yes_gate] BLOCK YES {c['ticker']} — "
                          f"adx={_adx_mid:.1f}∈[30,40) c_trend={_active_trend}<0 pm={pm:.3f}∈[0.70,0.85) "
                          f"rvol={_adx_rvol:.2f} R1={_adx_r1} (moderate trend against near-ITM YES)")
                    gate_audit_logger.log_block(
                        ...
                        signals={
                            ...
                            "composite_trend": _active_trend,
                            "rvol_1h":         round(_adx_rvol, 3),
                            "hmm_vol_state":   getattr(confirm, "hmm_vol_state", None),
                        },
                        now_utc=now_utc, bankroll=args.bankroll,
                    )

            # [stoch_oversold_yes_gate — BTC YES block]
            # Block YES when stoch_k_1h<10 AND chg_1h<0 (oversold + still falling).
            # Rescue baked in as AND condition: chg_1h>=0 = bounce already starting (n=1,155, +9.3pp edge).
            # Blocked set: n=8,177, WR=19.7%, bkev=50.1%, edge=−30.4pp. Snapshot MCPT z=+49.25, p=0.000.
            # Backup: paper_trade_runner_pre_stoch_rsi_gates_20260607.py
            if (args.asset == "BTC" and not _otm_yes_blocked
                    and _sk_1h_mtf < 10.0
                    and _chg_1h_mtf < 0.0):
                _otm_yes_blocked = True
                _otm_yes_block_gate = "stoch_oversold_yes_gate"
                print(f"  [stoch_oversold_yes_gate] BLOCK YES {c['ticker']} — "
                      f"stoch_k_1h={_sk_1h_mtf:.1f}<10 chg_1h={_chg_1h_mtf:+.3f}%<0 "
                      f"(oversold+still falling, edge=−30pp vs bkev)")
                gate_audit_logger.log_block(
                    gate_name="stoch_oversold_yes_gate",
                    ticker=c["ticker"], asset=args.asset, side="yes",
                    pm=pm, p_model=p_model_comp or float("nan"),
                    net_edge=max(0.0, (p_model_comp or 0.0) - pm - 0.04),
                    offset_pct=offset_c, strike=s_k, spot=spot, tau_minutes=tau_c,
