"""
S9 -- Retroactively reconstruct hmm_ms_state / hmm_vd_state (from ETH 1h
price history) and hmm_of_state (from sparse raw order-flow columns) for
the AGREE population, using the validated sequence-decode logic from the
2026-07-06 ms/vd/of HMM fix and ETH's own trained model files.
"""
import pickle
import numpy as np
import pandas as pd
...
c1h_full = pd.read_parquet(f"{REBUILD}/hist_ETHUSDT_1h.parquet").sort_index()

with open("models/hmm_microstructure_eth.pkl", "rb") as f:
    ms_pkg = pickle.load(f)
with open("models/hmm_voldirection_eth.pkl", "rb") as f:
    vd_pkg = pickle.load(f)
with open("models/hmm_orderflow_eth.pkl", "rb") as f:
...
if __name__ == "__main__":
    ...
    df = pd.read_csv(f"{OUT}/eth_agree_reconstructed.csv", low_memory=False)
    df["logged_at_parsed"] = pd.to_datetime(df["logged_at_parsed"], utc=True, errors="coerce")

    print("reconstructing ms/vd/of HMM states for ETH AGREE population...")
    ...
    for c in ["hmm_ms_state_recon", "hmm_vd_state_recon", "hmm_of_state_recon"]:
        print(f"  {c}: {df[c].notna().sum()}/{len(df)} non-null, values={sorted(df[c].dropna().unique().tolist())}")

    df.to_csv(f"{OUT}/eth_agree_fully_reconstructed.csv", index=False)
    print("saved")
