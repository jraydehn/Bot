...
def main() -> None:
    ...
    if auth is not None:
        ...
        for c in ladder:
            ...
            # Analysis May 2026 (1,210 settled paper trades):
            #   pm < 0.10              : 0W/20L — hard block, no rescue
            #   pm [0.10,0.20)+sk≤65  : 4W/33L (10.8% WR, -$355) — blocked, insufficient momentum
            #   pm [0.10,0.20)+sk>65  : 4W/8L  (33.3% WR, +$535) — rescue: strong momentum can reach strike
            #   pm [0.20,0.35)+ema>=1 : 0% WR  — EMA already stretched bullish, block
            #   pm [0.25,0.35)+sk>70  : 8% WR  — overbought into deep OTM, block
            # Net gate impact: +$1,085 vs baseline.
            if args.asset == "BTC" and _composite_computed and pm < 0.35:
                _ema_s    = confirm.ema_stretch_score
                # Use 1h stoch_k (14-bar = 14h lookback) — more robust than 15m which
                # hits 100 during any sustained uptrend, firing spuriously in trending regimes.
                ...
                if pm < 0.10:
                    _otm_yes_blocked = True
                    print(f"  [otm_yes_gate] BLOCK YES {c['ticker']} — pm={pm:.3f}<0.10 (hard block)")
                elif pm < 0.20:
                    _ema_bullish = (confirm.ema_stack_bias == 1)
                    if _sk_gate > 65 and _ema_bullish:
                        print(f"  [otm_yes_gate] RESCUE YES {c['ticker']} — pm={pm:.3f} in [0.10,0.20), stoch_k={_sk_gate:.1f}>65, ema_stack=1")
                    else:
                        _otm_yes_blocked = True
                        print(f"  [otm_yes_gate] BLOCK YES {c['ticker']} — pm={pm:.3f} in [0.10,0.20), stoch_k={_sk_gate:.1f} ema_stack={confirm.ema_stack_bias} (rescue requires sk>65+ema=1)")
                elif _ema_s >= 1:
                    _otm_yes_blocked = True
            ...
            if args.asset == "BTC" and _smc is not None:
                ...
                if (_smc.choch_4h and _smc.bos_1h == "bearish") or _smc.bos_4h == "bearish":
                    ...
                    print(f"  [smc_gate] BLOCK YES {c['ticker']} — 4h structure bearish "
                          f"(choch_4h={_smc.choch_4h}, bos_4h={_smc.bos_4h}, bos_1h={_smc.bos_1h})")
                    _smc_log("bearish_structure")
            ...
            # [G4 — SOL YES pm 0.30-0.50 mid-range gate]
            # Model overclaims: pmodel 0.52-0.60 but actual WR=35% (bkeven=41%) in this zone.
            # Rescue: ema_alignment=bearish (mean-reversion bounce play, n=6, 66.7% WR, $22.93/t)
            #      OR obi_score>0 (order-flow confirmation, n=4, 75% WR, $21.68/t).
            # Block (no rescue): n=12, 16.7% WR, -$29.54/t. Net save: +$354.
            if args.asset == "SOL" and dec_c.side == "yes" and 0.30 <= pm < 0.50:
                _g4_ema_bear = "bear" in _ema_align.lower()
                _g4_obi_pos  = confirm.obi_score > 0
                if not _g4_ema_bear and not _g4_obi_pos:
                    print(f"  [sol_yes_gate] BLOCK YES pm={pm:.3f} in [0.30,0.50) ema={_ema_align} obi={confirm.obi_score:.2f} — rescue requires ema=bear OR obi>0")
                    _log_block("sol_yes_gate")
                    continue
                else:
                    _g4_reasons = []
                    if _g4_ema_bear: _g4_reasons.append(f"ema={_ema_align}")
                    if _g4_obi_pos:  _g4_reasons.append(f"obi={confirm.obi_score:.2f}>0")
                    print(f"  [sol_yes_gate] RESCUE YES pm={pm:.3f} via {'+'.join(_g4_reasons)}")
            ...
            # [sol_no_struct_ema_gate] Block SOL NO when structure_bias=-1 (bearish structure)
            # but ema_stack_bias != -1 (EMA trend not confirming bearish direction).
            # Without EMA alignment, bearish structure alone is insufficient for NO conviction.
            # Rescue: ema_stack_bias=-1 = confirmed fully bearish EMA stack validates the structure.
            # Analysis: structure_bias=-1 segment saves ~$908; rescue recovers aligned-trend wins.
            if (args.asset == "SOL"
                    and dec_c.side == "no"
                    and struct.structure_bias == -1):
                if confirm.ema_stack_bias != -1:
                    print(f"  [sol_no_struct_ema_gate] BLOCK NO {c['ticker']} — "
                          f"structure_bias=-1 (bearish) but ema_stack_bias={confirm.ema_stack_bias} (not confirmed)")
                    _log_block("sol_no_struct_ema_gate")
                    continue
                else:
                    print(f"  [sol_no_struct_ema_gate] RESCUED NO {c['ticker']} — "
                          f"structure_bias=-1 + ema_stack_bias=-1 (bearish aligned)")

            # [sol_no_trend_struct_gate] Block SOL NO when composite_trend>=3 (strong bullish)
            # AND structure_bias != 1. Fighting a strong trend without structural confirmation
            # (supply zone / resistance ceiling) has no edge.
            # Rescue: structure_bias=1 = bullish structure confirms a ceiling above current price.
            # Analysis: trend>=3 segment saves ~$783; structure=1 rescue recovers zone-limited wins.
            if (args.asset == "SOL"
                    and dec_c.side == "no"
                    and _active_trend >= 3):
                if struct.structure_bias != 1:
                    print(f"  [sol_no_trend_struct_gate] BLOCK NO {c['ticker']} — "
                          f"composite_trend={_active_trend}>=3 (bullish) + structure_bias={struct.structure_bias} (no ceiling)")
                    _log_block("sol_no_trend_struct_gate")
                    continue
                else:
                    print(f"  [sol_no_trend_struct_gate] RESCUED NO {c['ticker']} — "
                          f"trend={_active_trend}>=3 but structure_bias=1 (bullish ceiling confirmed)")
            ...
            # [BTC NO z_abs gate] Block NO bets where the strike is < 0.45σ from spot.
            # History: 0.30 → 0.60 (dual-model reform), 0.60 → 0.45 (2026-05-07 sim):
            # paper_trades backtest (n=450 resolved): threshold=0.45 gives +$408 vs
            # threshold=0.60 +$224. The 0.45-0.60 bucket (n=70, WR=67.1%, BE=66.5%)
            # is genuinely profitable and was incorrectly blocked at 0.60.
            if (args.asset == "BTC" and dec_c.side == "no"
                ...
                if _z_no < 0.45:
                    print(f"  [btc_no_z_gate] BLOCK NO {c['ticker']} — |z|={_z_no:.3f} < 0.45 (near-ATM, no structural edge)")
            ...
            # Rescue: vpin_raw >= 0.75 AND p_market >= 0.38 AND offset_pct <= 0.08
            #   (smart money absorbing cascade at near-ATM level — revival possible)
            # Backtest: n=104 blocked (WR=18.3%, BE=31.7%, Δ=-13.4%, -$1,872)
            #           n=12 rescued (WR=66.7%, BE=46.5%, Δ=+20.2%, +$258)
            #           Hard block n=92 saves $2,130  |  net vs flat: +$258
            ...
            if (dec_c.decision == "trade" and args.asset == "BTC"
                ...
                _lc_rescue = (_lc_vpin and pm >= 0.38 and offset_c <= 0.08)
                if _lc_rescue:
                    ...
                    _lc_why = (f"vpin={confirm.vpin_raw:.3f}<0.75" if not _lc_vpin
                               else f"pm={pm:.3f}<0.38" if pm < 0.38
                               else f"offset={offset_c*100:.2f}%>8%")
                    print(f"  [liq_cascade_gate] BLOCK YES {c['ticker']} — "
                          f"liq_score={_liq_signal.liq_score:+d} ({_liq_signal.label}) "
                          f"OTM={offset_c*100:.2f}%  no rescue ({_lc_why})")
