        ...
        for c in ladder:
            ...
            if _composite_computed:
                ...
                if args.asset == "BTC" and _composite_computed:
                    ...
                    # YES drift: norm.ppf(comp_p_up_c) × rvol_inv × k=0.3 × √(τ/60)
                    if sigma_tau_c > 0:
                        from scipy.stats import norm as _norm_pup
                        _z_drift_6h = float(_norm_pup.ppf(max(0.01, min(0.99, _comp_p_up_c)))) * _rvol_inv_btc * 0.3 * math.sqrt(tau_c / 60.0)
            ...
            if args.asset == "BTC" and _composite_computed and sigma_tau_c > 0:
                ...
                if _sigma_tau_no > 0:
                    ...
                    _z_drift_no = float(_norm_no.ppf(max(0.01, min(0.99, _comp_p_up_c)))) * _rvol_inv_btc * 0.3 * _sq_no
                else:
                    _z_drift_no = 0.0
